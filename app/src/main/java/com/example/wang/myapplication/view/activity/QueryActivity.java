...
import com.example.wang.myapplication.presenter.MainPresenter;
import com.example.wang.myapplication.utils.DensityUtil;
...
import com.example.wang.myapplication.utils.tools.StringUtil;
import com.example.wang.myapplication.view.fragment.RightDialogFragment;
import com.example.wang.myapplication.view.fragment.SettingDialogFragment;
import com.example.wang.myapplication.view.view.BaseView;
import com.example.wang.myapplication.view.view.MainLinearLayout;
import com.example.wang.myapplication.view.view.MainScrollView;
import com.example.wang.myapplication.view.view.MainToolbar;
...
public class QueryActivity extends AppCompatActivity implements BaseView,
        MainToolbar.OnRightClickListener,MainToolbar.OnLeftClickListener,View.OnClickListener{

    private static final String TAG = QueryActivity.class.getSimpleName() + "-TAG";
    ...
    private void initData(){
        ...
        if (getIntent().getStringExtra("curr_date")!=null){
            ...
            MainData mainData = Application.databaseUtil.getData(Integer.parseInt(currDate));
    ...
    public void initSizeColor(){
        ...
        title.setTextSize(Config.textSizesTitle[textSizeIdx]);
        author.setTextSize(Config.textSizesAuthor[textSizeIdx]);
        footer.setTextSize(Config.textSizesAuthor[textSizeIdx]);
        content.setTextSize(Config.textSizesContent[textSizeIdx]);
        if (!isNightMode){
            topLine.setBackgroundColor(Color.rgb(228,219,219));
            bottomLine.setBackgroundColor(Color.rgb(228,219,219));
            title.setTextColor(Config.textColors[colorIdx]);
            author.setTextColor(Config.textColors[colorIdx]);
            footer.setTextColor(Config.textColors[colorIdx]);
            content.setTextColor(Config.textColors[colorIdx]);
            mainLayout.setBackgroundColor(Config.colors[colorIdx]);
            articleLayout.setBackgroundColor(Config.colors[colorIdx]);
        }else {
            topLine.setBackgroundColor(Color.rgb(70,70,70));
            bottomLine.setBackgroundColor(Color.rgb(70,70,70));
            title.setTextColor(Config.nightTextColor);
            author.setTextColor(Config.nightTextColor);
            footer.setTextColor(Config.nightTextColor);
            content.setTextColor(Config.nightTextColor);
            mainLayout.setBackgroundColor(Config.nightBackColor);
            articleLayout.setBackgroundColor(Config.nightBackColor);
        }
    ...
    protected void onResume() {
        super.onResume();
        MiStatInterface.recordPageStart(this, "每日一文");
        initData();
