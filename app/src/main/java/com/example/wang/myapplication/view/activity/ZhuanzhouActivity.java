...
public class ZhuanzhouActivity extends AppCompatActivity {
	...
	public void inti() {
		...
		String str =  com.example.wang.myapplication.utils.FileTools.getFile(context, "zhuanzhou.txt");
		if (str == null) {
			 com.example.wang.myapplication.utils.Toast.show("读取文件失败");
			return;
		} else {
			if(str.length() == 0){
				 com.example.wang.myapplication.utils.Toast.show("你们没有专周耶~");
				return;
			}
			// 适配数据
