...
public class LoginSchool_Activity extends AppCompatActivity {
    ...
    private void success() {
        ...
        FileTools.saveshareString("login","true");
         ...
         com.example.wang.myapplication.view.activity.MainActivity.instance.finish();
        startActivity(new Intent(context,  com.example.wang.myapplication.view.activity.MainActivity.class));
        finish();
...
//        builder.show();
    ...
    class RefreshListener implements View.OnClickListener {
        ...
        private void login() {
            ...
            // 加密密码
            //单独对密码进行MD5加密，并在取第1-30位摘要字符串后将全部的字母转化成大写（从程序的角度应该是0-29位）；
            //上述步骤生成的摘要值与学号字符串相加后再加一个“10919”字符串，并将该合成字符串再一次MD5加密，继续提取1-30位，继续转化成大写；
            final String Tmm = GetMD5(username+GetMD5(password).substring(0,30).toUpperCase()+"10919").substring(0,30).toUpperCase();
            ...
            new Thread() {
                public void run() {
                    String result = LoginService.login(username, Tmm);
                                ...
                                public void done(User user, BmobException e) {
                                    if (user != null) {
                                        ...
                                        User userInfo = BmobUser.getCurrentUser(User.class);
                                         com.example.wang.myapplication.Application.setuser(userInfo);
                                        Message mes = new Message();
                                        for (i = 55; i < 100; i++) {
                                            try {
                                                mes = mes.obtain();
